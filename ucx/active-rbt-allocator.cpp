...
Color ActiveAllocator::readColor(NodeIndex Ni) {
  size_t Offset = Ni.getValue() * sizeof(Node) + offsetof(Node, color);

  Color ColorValue;

  ucs_status_t status = ucp_get(ep, &ColorValue, sizeof(Color),
                                BaseAddressNodes + Offset, NodesKey);
  if (status != UCS_OK)
    printf("failed to get (%s)\n", ucs_status_string(status));
  assert(status == UCS_OK);
...
void ActiveAllocator::writeColor(NodeIndex Ni, Color c) {
  size_t Offset = Ni.getValue() * sizeof(Node) + offsetof(Node, color);

  Color TheColor = c;

  ucs_status_t status = ucp_put(ep, &TheColor, sizeof(Color),
                                BaseAddressNodes + Offset, NodesKey);
