...
void PassivePMDKAllocator::initPMDK() {
  // open nodes file
  {
    std::string path = "/mnt/pmem0/bzcschut/ucx/nodes.bin";
  ...
  {
    std::string path = "/mnt/pmem0/bzcschut/ucx/log.bin";
    size_t size = sizeof(RBTLogStructure);
    size_t mapped_len = 0;

    LogP = static_cast<RBTLogStructure *>(
        pmem_map_file(path.c_str(), size, PMEM_FILE_CREATE, S_IWUSR | S_IRUSR,
                      &mapped_len, nullptr));
    assert(LogP != nullptr);
    assert(mapped_len == size);
  ...
  {
    std::string path = "/mnt/pmem0/bzcschut/ucx/state.bin";
  ...
  {
    std::string path = "/mnt/pmem0/bzcschut/ucx/current.bin";
